...
export default function TTSAgentsPage() {
  ...
  const [integrationsList, setIntegrationsList] = useState<Integration[]>(() =>
    INITIAL_MOCK_INTEGRATIONS.filter((i) => i.category === "TTS Agent")
  );
    ...
    <RouteGuard module="integrations">
      <div className="space-y-6 pb-12">
        ...
        <div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-1 border-b border-border/60">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20 shadow-2xs">
                <Volume2 className="h-5 w-5" />
              </div>
              <div>
                <h1 className="text-xl font-bold tracking-tight text-foreground">
                  Text-to-Speech (TTS) Agents
                </h1>
                <p className="text-xs text-muted-foreground">
                  Configure ultra-low-latency neural speech synthesis and voice generation models.
                </p>
              </div>
            </div>
          </div>
        ...
        <IntegrationToolbar
          ...
          currentRoute="tts"
          createButtonText="Create TTS Agent"
